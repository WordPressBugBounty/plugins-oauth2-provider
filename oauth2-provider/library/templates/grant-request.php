...
if ( isset( $_POST['request-grant'] ) ) {
	if ( wp_verify_nonce( $_POST['nonce'], 'grant-request' ) ) {
		...
		if ( $user_response == 'allow' ) {
			update_user_meta( get_current_user_id(), 'wo_grant_' . sanitize_text_field( $_REQUEST['client_id'] ), 'allow' );
		} elseif ( $user_response == 'deny' ) {
			update_user_meta( get_current_user_id(), 'wo_grant_' . sanitize_text_field( $_REQUEST['client_id'] ), 'deny' );
		}
	}
...
$client = get_client_by_client_id( sanitize_text_field( $_REQUEST['client_id'] ) );
?>
<style>
	body {
		background: #ecf0f1;
